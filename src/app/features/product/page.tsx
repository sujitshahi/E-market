...
async function safeFetchProducts(signal: AbortSignal) {
  try {
    ...
    return { data: null, error: 'Unable to load products. Please check your internet connection.', aborted: false }
  }
}
...
export default function Page() {
  ...
  const toggleTheme = () => {
    ...
    setIsDark(nextTheme)
    localStorage.setItem('theme:v1', nextTheme ? 'dark' : 'light')
  }
  ...
  const loadProducts = useCallback(async (signal?: AbortSignal) => {
    ...
    const result = await safeFetchProducts(activeSignal)
    ...
    if (result.error) {
      ...
      const fetchedProducts: Product[] = result.data.products || []
  ...
  return (
    <div
      className={`min-h-screen transition-colors duration-500 py-12 px-4 sm:px-6 lg:px-8 selection:bg-indigo-500 selection:text-white ${
    ...
    >
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <header className="text-center space-y-4 max-w-2xl mx-auto">
          <div
            className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-semibold uppercase tracking-widest backdrop-blur-md ${
              isDark
          ...
          >
            From trending electronics and beauty products to everyday essentials—explore our full catalog with instant
            search and live category filters.
          </p>
        </header>

        {error && (
          <div
            className={`border rounded-3xl p-12 text-center max-w-md mx-auto space-y-4 ${
        ...
        {!error && (
          <>
            <div
              className={`backdrop-blur-xl border rounded-3xl p-4 shadow-2xl flex flex-wrap gap-4 justify-between items-center transition-colors ${
            ...
            >
              ...
              <div className="relative flex-1 sm:flex-none w-full sm:w-auto">
                ...
                >
                  <option value="default" className={isDark ? 'bg-slate-900' : 'bg-white'}>
                    Sort by: Featured
                  </option>
                  <option value="price-low" className={isDark ? 'bg-slate-900' : 'bg-white'}>
                    Price: Low to High
                  </option>
                  <option value="price-high" className={isDark ? 'bg-slate-900' : 'bg-white'}>
                    Price: High to Low
                  </option>
                  <option value="popularity" className={isDark ? 'bg-slate-900' : 'bg-white'}>
                    Customer Rating
                  </option>
                </select>
            ...
            {loading && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className={`border rounded-3xl p-4 space-y-4 animate-pulse ${
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {displayedProducts.map((product) => (
                  ...
                  >
                    <div>
                      <div
                        className={`relative aspect-square w-full overflow-hidden rounded-2xl mb-4 border ${
                      ...
                      >
                        ...
                        <div className="absolute top-3 left-3 right-3 flex justify-between items-center pointer-events-none z-10">
                          {product.category && (
                            <span
                              className={`text-[10px] font-bold px-2.5 py-1 rounded-full border capitalize backdrop-blur-md ${
                                isDark
